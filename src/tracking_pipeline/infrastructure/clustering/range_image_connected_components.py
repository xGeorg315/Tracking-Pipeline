...
from tracking_pipeline.infrastructure.clustering.sensor_space import range_jump_connectivity


class RangeImageConnectedComponentsClusterer:
    ...
    def cluster(self, frame: FrameData, lane_box: LaneBox) -> ClusterResult:
        ...
        if len(lane_points) < self.config.vehicle_min_points:
            return ClusterResult(
                ...
                detections=[],
                metrics={"algorithm": "range_image_connected_components", "lane_point_count": int(len(lane_points))},
        ...
        return build_sensor_cluster_result(
            ...
            batches,
            self.config,
            connect_fn=range_jump_connectivity(self.config, scale=1.5),
        )
